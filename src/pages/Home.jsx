...
const Home = () => {
  const hotCaps = hotProducts.filter(p => p.id.startsWith('hot-') && parseInt(p.id.split('-')[1]) <= 4);
  const hotWallets = hotProducts.filter(p => p.id.startsWith('hot-') && parseInt(p.id.split('-')[1]) >= 5);
